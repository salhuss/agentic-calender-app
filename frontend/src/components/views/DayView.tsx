  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {allDayEvents.length > 0 && (
        <div className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-850 p-4">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">All Day</h3>
          <div className="space-y-2">
            {allDayEvents.map((event) => (
              <div
                key={event.id}
                className="bg-blue-100 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg p-3 cursor-pointer hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors"
                onClick={() => onEditEvent(event)}
              >
                <div className="font-medium text-blue-900 dark:text-blue-100">{event.title}</div>
                {event.description && (
                  <div className="text-sm text-blue-700 dark:text-blue-300 mt-1">{event.description}</div>
                )}
                {event.location && (
                  <div className="text-sm text-blue-600 dark:text-blue-400 mt-1">📍 {event.location}</div>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 overflow-y-auto bg-white dark:bg-gray-900">
        {timedEvents.length === 0 && allDayEvents.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-500 dark:text-gray-400">
            ...
            <p className="text-lg font-medium mb-2 text-gray-700 dark:text-gray-300">No events scheduled</p>
          ...
          <div className="p-4 space-y-3">
            {timedEvents
              ...
              .map((event) => (
                <div
                  key={event.id}
                  className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm cursor-pointer hover:shadow-md hover:border-gray-300 dark:hover:border-gray-600 transition-all"
                  onClick={() => onEditEvent(event)}
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="w-4 h-4 bg-blue-500 dark:bg-blue-400 rounded-full flex-shrink-0 mt-1"></div>
                  </div>
                </div>
              ))}
          </div>
